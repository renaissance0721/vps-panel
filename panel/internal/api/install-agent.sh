...
usage() {
  cat <<'EOF'
Usage: install-agent.sh --server PANEL_URL --token ENROLLMENT_TOKEN [--force]

Options:
  --server URL   Public URL of the VPS Panel.
  --token TOKEN  One-time enrollment token generated by the Panel.
  --force        Replace an existing Agent identity after registration succeeds.
...
if [[ -e "$CONFIG_FILE" && "$force" != true ]]; then
  fail "an Agent is already registered on this VPS; use --force to rebind it"
fi
...
curl --proto '=https' --tlsv1.2 -fL --retry 3 --retry-delay 2 \
  "${RELEASE_DOWNLOAD_BASE}/${asset_name}" \
  -o "$download_path"
...
registration_arguments=(register --server "$server_url" --token "$enrollment_token")
if [[ "$force" == true ]]; then
  registration_arguments+=(--force)
fi
"$download_path" "${registration_arguments[@]}"
enrollment_token=""
