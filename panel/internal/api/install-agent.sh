...
usage() {
  cat <<'EOF'
Usage: install-agent.sh --server PANEL_URL --token ENROLLMENT_TOKEN
...
for command_name in curl uname systemctl install mktemp; do
  command -v "$command_name" >/dev/null 2>&1 || fail "required command not found: ${command_name}"
done
[[ -d /run/systemd/system ]] || fail "systemd is not running on this VPS"
[[ ! -e "$CONFIG_FILE" ]] || fail "an Agent is already registered on this VPS"
...
"$BINARY_PATH" register --server "$server_url" --token "$enrollment_token"
enrollment_token=""
