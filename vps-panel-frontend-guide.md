...
概览顶部紧凑展示已注册账号（仅用户名和 admin / vip 等级）、当前账号可访问的未归档 Server 数和 Proxy 数。私有资源仍按 Server 授权范围计入，admin 不自动绕过权限；账号较多时在卡片内滚动。下方保留运行状态与仅 admin 可见的邀请区域，隐藏邀请区域后网格自动填充。
...
Server、Proxy、Relay 主列表在最左侧使用窄排序列和小型 ↑ ↓ 按钮，操作列仍在最右侧。上移、下移只针对当前账号实际可见的完整列表；Server 正常与已移除列表分别移动。排序偏好按账号持久化，不修改资源状态、desired state 或 Agent 运行配置。搜索关键词非空时禁用箭头，避免筛选结果产生含糊的相邻关系。
