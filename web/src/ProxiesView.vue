...
async function saveProxy() {
  if (!proxyName.value.trim() || !proxyServerName.value.trim()) {
    error.value = '请填写节点名称和 SNI'
    return
  }
    ...
    if (proxyFormMode.value === 'create') {
      const response = await api<{ proxy: ProxyRecord }>('/api/proxies', {
        ...
        body: JSON.stringify({
          ...
          first_client_udp443: firstClientUDP443.value,
        }),
      })
...
async function saveClient() {
  ...
  await run(async () => {
    const body = JSON.stringify({
      ...
      client_udp443: clientUDP443.value,
    })
    if (clientFormMode.value === 'create') {
      await api(`/api/proxies/${selectedProxy.value?.id}/clients`, { method: 'POST', body })
    } else {
...
async function copyValue(type: 'uuid' | 'uri', value: string) {
  try {
...
<template>
      ...
      <table class="server-table proxy-table">
        ...
        <tbody>
          <tr v-for="value in filteredProxies" :key="value.id">
            ...
            <td>VLESS</td>
            <td>TCP</td>
            <td>{{ value.config.security === 'tls' ? 'TLS' : 'REALITY' }}</td>
            <td>XTLS Vision</td>
    ...
    <n-card class="proxy-form-card" :title="proxyFormMode === 'create' ? '新增代理节点' : '编辑代理节点'" :bordered="false" closable @close="proxyFormOpen = false">
      <form class="proxy-form" @submit.prevent="saveProxy">
        ...
        <fieldset v-if="proxyFormMode === 'create'" class="client-fieldset">
          ...
          <p>UUID 将由后端安全生成。</p>
          <label class="checkbox-row"><input v-model="firstClientUDP443" type="checkbox" /><span>允许 UDP/443 / QUIC</span></label>
        </fieldset>
  ...
  <n-modal v-if="selectedProxy" v-model:show="proxyDetailOpen">
    <n-card class="proxy-detail-card" title="代理节点详情" :bordered="false" closable @close="proxyDetailOpen = false">
      ...
      <h3>协议</h3>
      <dl class="server-details"><div><dt>协议</dt><dd>VLESS</dd></div><div><dt>传输</dt><dd>TCP</dd></div><div><dt>流控</dt><dd>XTLS Vision</dd></div><div><dt>安全层</dt><dd>{{ selectedProxy.config.security === 'tls' ? 'TLS' : 'REALITY' }}</dd></div></dl>
      <h3>{{ selectedProxy.config.security === 'tls' ? 'TLS' : 'REALITY' }}</h3>
      <dl class="server-details">
  ...
  <n-modal v-model:show="clientFormOpen" :mask-closable="!submitting">
    <n-card class="client-form-card" :title="clientFormMode === 'create' ? '新增客户端' : '编辑客户端'" :bordered="false" closable @close="clientFormOpen = false">
      <form class="proxy-form" @submit.prevent="saveClient">
        ...
        <p>UUID 由后端生成，创建后保持不变。</p>
        <label class="checkbox-row"><input v-model="clientUDP443" type="checkbox" /><span>允许 UDP/443 / QUIC</span></label>
  ...
  <n-modal v-if="selectedShare" v-model:show="clientDetailOpen">
    <n-card class="client-detail-card" title="客户端详情" :bordered="false" closable @close="clientDetailOpen = false">
      <dl class="server-details">
        ...
        <div><dt>UUID</dt><dd>{{ selectedShare.client.uuid }}</dd></div><div><dt>客户端 Flow</dt><dd>{{ selectedShare.flow }}</dd></div>
        <div><dt>连接地址</dt><dd>{{ selectedShare.address }}</dd></div><div><dt>端口</dt><dd>{{ selectedShare.port }}</dd></div>
        <div><dt>安全层</dt><dd>{{ selectedShare.security === 'tls' ? 'TLS' : 'REALITY' }}</dd></div><div><dt>SNI</dt><dd>{{ selectedShare.server_name }}</dd></div>
        <template v-if="selectedShare.security === 'reality'"><div><dt>Public Key</dt><dd>{{ selectedShare.reality_public_key }}</dd></div><div><dt>Short ID</dt><dd>{{ selectedShare.reality_short_id }}</dd></div></template>
      </dl>
      <div class="share-field"><strong>直连 VLESS URI</strong><n-input :value="selectedShare.uri" type="textarea" readonly :autosize="{ minRows: 4 }" /></div>
      <div class="modal-actions"><n-button secondary @click="copyValue('uuid', selectedShare.client.uuid)">{{ copied === 'uuid' ? 'UUID 已复制' : '复制 UUID' }}</n-button><n-button type="primary" @click="copyValue('uri', selectedShare.uri)">{{ copied === 'uri' ? '链接已复制' : '复制 VLESS 链接' }}</n-button></div>
    </n-card>
  </n-modal>
</template>
