...
test('Server owner 与删除 UI 保持列表简洁并在详情保留管理员强制移除', async () => {
  ...
  assert.match(types, /owner_user_id: number \| null/)
  assert.match(types, /owner_username: string/)
  ...
  assert.match(composable, /清理成功后服务器才会进入“已移除”/)
  assert.match(composable, /确认后开始删除/)
  assert.match(composable, /强制移除不会清理远端 VPS。远端可能继续运行 Xray、Realm、Agent 和监听端口。/)
  assert.match(composable, /\/api\/servers\/\$\{value\.id\}\/force/)
  assert.match(list, /<th>所有者<\/th>/)
  assert.match(list, /value\.owner_username \|\| '—'/)
  assert.match(list, /等待 Agent 上线清理/)
  assert.match(list, /正在删除/)
  assert.match(list, /删除失败/)
  assert.match(list, /删除中/)
  assert.doesNotMatch(list, /开始退役|强制从 Panel 移除/)
  assert.match(detail, /<dt>所有者<\/dt>/)
  assert.match(detail, /selectedServer\.owner_username \|\| '—'/)
  assert.match(detail, /正在删除/)
  assert.match(detail, /删除失败/)
...
test('中国 IP 入站开关只出现在详情并保留原 PATCH API', async () => {
  ...
  for (const column of ['排序', '名称', '所有者', '状态', '本周期流量', '到期时间', '操作']) assert.match(list, new RegExp(column))
